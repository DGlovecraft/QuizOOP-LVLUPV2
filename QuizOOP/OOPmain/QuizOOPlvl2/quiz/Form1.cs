...
        private List<Student> StudentsDAta = new List<Student>();
        ...
        public List<Teacher> TeachersA = new List<Teacher>();

        public List<Teacher> TeachersB = new List<Teacher>();

        public List<Teacher> TeachersC = new List<Teacher>();

        public List<Teacher> TeachersD = new List<Teacher>();
...

        {
            ...
            InitializeComponent();
            TeachersA.Add(new Teacher("Dr. bob", "Logistic", "Math"));  // อาจารย์ในกลุ่ม A
            TeachersB.Add(new Teacher("Dr. Pront Johndan", "Computer Science", "eng"));  // อาจารย์ในกลุ่ม B
            TeachersC.Add(new Teacher("Dr. Jin", "Biology", "Genetics"));  // อาจารย์ในกลุ่ม C
        }
        ...
        {
            List<Teacher> selectedTeachers = null;

            // Get the selected advisor group from comboBox3
            string selectedGroup = comboBox3.Text;

            if (selectedGroup == "Dr_bob") selectedTeachers = TeachersA;
            else if (selectedGroup == "Dr_Pront Johndan") selectedTeachers = TeachersB;
            else if (selectedGroup == "Dr_Jin") selectedTeachers = TeachersC;
...

        {
            ...
            Teacher TeacherAdd = new Teacher("Dr. bob", "Math", "Physics");
...

            {
...

                {
                    ...
                    if (advisor_enum == Student_advisor_ForStudent.Dr_bob)
                    {
                        // เมื่อ advisor_enum เป็น TeacherA
                        Teacher teacher = new Teacher(student.name, subject1, subject2); // สร้าง Teacher โดยส่งชื่อวิชา
                        TeachersA.Add(teacher);
                    }
                    else if (advisor_enum == Student_advisor_ForStudent.Dr_ProntJohndan)
                    {
                        Teacher teacher = new Teacher(student.name, subject1, subject2);
                        TeachersB.Add(teacher);
                    }
                    else if (advisor_enum == Student_advisor_ForStudent.Dr_Jin)
                    {
                        Teacher teacher = new Teacher(student.name, subject1, subject2);
